# taken from https://raw.githubusercontent.com/asottile/astpretty/master/astpretty.py, modified to allow recursion limit 
# on pformat to avoid overly large output for interactive / "visualization" modes
...
def pformat(
    ...
    if node is None:
        ...
        def _pformat(el: Union['ASTType', None, str], _indent: int = 0, _depth: int = 0) -> str:
            return pformat(
                el, indent=indent, show_offsets=show_offsets,
                _indent=_indent, _depth=_depth, max_depth=max_depth
            )
        ...
        with indented():
            for field in _fields(node, show_offsets=show_offsets):
                ...
                if attr == []:
                    ...
                    representation = f'[{_pformat(attr[0], _depth = _depth + 1)}]'
                elif isinstance(attr, list):
                    representation = '[\n'
                    with indented():
                        for el in attr:
                            representation += '{}{},\n'.format(
                                indentstr(), _pformat(el, state.indent, _depth = _depth + 1),
                            )
                    representation += indentstr() + ']'
                elif isinstance(attr, AST):
                    representation = _pformat(attr, state.indent, _depth = _depth + 1)
                else:
